...
module RenderedElement: {
  ...
  let render:
    ('node, hostNodeElement('node, 'childNode), element('childNode)) =>
    t('node, 'childNode);
...
let nativeComponent:
  (
    ...
    string,
    ~key: Key.t=?,
    Hooks.t('a, 'a) => (Hooks.t(Hooks.nil, 'a), hostNodeElement('node, 'childNode))
  ) =>
  element('node);
...
module Expert: {
  ...
  let nativeComponent:
    (
      ...
      string,
      ~key: Key.t=?,
      Hooks.t('a, 'a) => (hostNodeElement('node, _), Hooks.t(Hooks.nil, 'a))
    ) =>
    element('node);
};
